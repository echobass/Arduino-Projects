...
Adafruit_NeoPixel pixels = Adafruit_NeoPixel(7, PIN, NEO_GRBW + NEO_KHZ800);
...
void setup() {
  pixels.begin();
  pixels.setBrightness(8); // 1/3 brightness
  prevTime = millis();
}
...
void rainbowCycleslow(uint8_t wait) {
  ...
  for(j=0; j<256*3; j++) { // 3 cycles of all colors on wheel
    for(r=1; r< pixels.numPixels(); r++) {
      pixels.setPixelColor(r, Wheel(((r * 256 / (pixels.numPixels()-1)) + j) & 255));
    }
